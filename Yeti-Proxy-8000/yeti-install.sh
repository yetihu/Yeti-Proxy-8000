...
# Author: Yeti
# Github: https://github.com/yetihu/Yeti-Proxy-8000/
############################################################
...
/usr/bin/wget -q --no-check-certificate -O /usr/local/bin/sok-find-os https://raw.githubusercontent.com/yetihu/Yeti-Proxy-8000/main/sok-find-os.sh > /dev/null 2>&1
chmod 755 /usr/local/bin/sok-find-os

/usr/bin/wget -q --no-check-certificate -O /usr/local/bin/squid-uninstall https://raw.githubusercontent.com/yetihu/Yeti-Proxy-8000/main/squid-uninstall.sh > /dev/null 2>&1
chmod 755 /usr/local/bin/squid-uninstall

/usr/bin/wget -q --no-check-certificate -O /usr/local/bin/adv-add-user https://raw.githubusercontent.com/yetihu/Yeti-Proxy-8000/main/yeti-add.sh > /dev/null 2>&1
chmod 755 /usr/local/bin/adv-add-user

if [[ -d /etc/squid/ || -d /etc/squid3/ ]]; then
    echo "Yeti Proxy already installed."
    exit 1
fi
...
if [ $SOK_OS == "ubuntu2204" ]; then
    ...
    /usr/bin/wget -q --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/yetihu/Yeti-Proxy-8000/main/conf/ubuntu-2204.conf
    ...
    /usr/bin/touch /etc/squid/blacklist.acl
    /usr/bin/wget -q --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/yetihu/Yeti-Proxy-8000/main/squid.conf
    if [ -f /sbin/iptables ]; then
        /sbin/iptables -I INPUT -p tcp --dport 8000 -j ACCEPT > /dev/null 2>&1
    fi
    service squid restart > /dev/null 2>&1
    ...
    /usr/bin/wget -q --no-check-certificate -O /etc/squid3/squid.conf https://raw.githubusercontent.com/yetihu/Yeti-Proxy-8000/main/squid.conf
    /sbin/iptables -I INPUT -p tcp --dport 8000 -j ACCEPT > /dev/null 2>&1
    ...
    /usr/bin/apt -y install apache2-utils squid  > /dev/null 2>&1
    touch /etc/squid/passwd
    /usr/bin/wget -q --no-check-certificate -O /etc/squid/conf.d/serverok.conf https://raw.githubusercontent.com/yetihu/Yeti-Proxy-8000/main/conf/debian12.conf
    if [ -f /sbin/iptables ]; then
        /sbin/iptables -I INPUT -p tcp --dport 8000 -j ACCEPT > /dev/null 2>&1
    fi
    systemctl enable squid
    ...
    /bin/rm -f /etc/squid/squid.conf
    /usr/bin/touch /etc/squid/blacklist.acl
    /usr/bin/wget -q --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/yetihu/Yeti-Proxy-8000/main/conf/squid-centos7.conf
    systemctl enable squid > /dev/null 2>&1
...
sed -i 's/Squid proxy-caching web server/Yeti Proxy Service/g'  /etc/squid/squid.conf

systemctl restart squid > /dev/null 2>&1
systemctl restart squid3 > /dev/null 2>&1

echo -e "${NC}"
echo -e "${GREEN}Thank you for using Yeti Proxy Service.${NC}"
echo
