...
if [ $SOK_OS == "ERROR" ]; then
    ...
    echo "Contact https://t.me/hibiscus_3 to add support for your OS."
    exit 1;
fi
...
rm -f /root/yeti-install.sh > /dev/null 2>&1
rm -f /usr/local/bin/sok-find-os > /dev/null 2>&1
rm -f /usr/local/bin/squid-uninstall > /dev/null 2>&1

echo 
echo 
echo "Yeti Proxy uninstalled."
echo "Thank you for using Yeti proxy installer"
echo "If you want to reinstall Yeti Proxy Server, check https://github.com/yetihu/Yeti-Proxy-8000/"
echo 
